...
{
    m_radius = GoBiggerConfig::massToRadius(m_mass);
}
...
void BaseBall::move(const QVector2D& direction, qreal duration)
{
    ...
    if (direction.length() > 0.01) {
        // 计算基于质量的最大速度
        float maxSpeed = GoBiggerConfig::calculateSpeed(m_mass);
        
        // 设置加速度朝向目标方向
        QVector2D targetVelocity = direction.normalized() * maxSpeed;
        QVector2D accel = (targetVelocity - m_velocity) * 5.0f; // 加速度系数
        
        setAcceleration(accel);
    } else {
        // 没有输入时应用阻力
        setAcceleration(-m_velocity * 2.0f);
    }
...
bool BaseBall::canEat(BaseBall* other) const
{
    ...
    // 使用GoBigger标准吞噬比例
    bool canEatResult = GoBiggerConfig::canEat(m_mass, other->mass());
